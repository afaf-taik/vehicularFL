...
def flip_labels_clusters(args, user_groups,dataset_train, dataset_test, n_clusters = 4 ,original_labels= [1,7,3,5], alternative_labels=[7,1,5,3]):
    dataset_train1 = []
    dataset_test1 = []
    clusters = []
...
# def models_similarity(args, l):
#     similarities = np.zeros((len(l),len(l)))
#     print(similarities)
#     cos = torch.nn.CosineSimilarity(dim=1, eps=1e-6)
#     for i in range(len(l)):
#         for j in range(i, len(l)): 
#             similarities[i][j] = cos(l[i].tolist(), l[j].tolist())
...
def models_similarity(l):
    ...
    for i in range(len(l)):
        for j in range(i, len(l)):
            similarities[i][j] = torch.sum(weights[i]*weights[j])/(torch.norm(weights[i])*torch.norm(weights[j])+1e-12) 
            similarities[j][i] = similarities[i][j]
    return similarities